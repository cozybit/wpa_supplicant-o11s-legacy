			    ...
			    u8 auth_transaction)
{
	...
	if (auth_transaction == 1) {
		...
		/* commit in ACCEPTED state indicates a reauth */
		...
		/* already in SAE_ACCEPTED, resend confirm frame */
		data = auth_build_sae_confirm(hapd, sta);
		if (data == NULL)
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
	} else {
		...
		if (hapd->conf->mesh & MESH_ENABLED) {
			if (sta->wpa_sm == NULL)
				sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
								sta->addr, NULL);
			if (sta->wpa_sm == NULL) {
				wpa_printf(MSG_DEBUG, "SAE: Failed to initialize WPA "
					   "state machine");
				resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
				goto fail;
			}
